...
import it.infocert.demoportal.restclient.TopClient;
...
@Path("/top")
public class TopResourceServices4OCE {

    @RestClient
    TopClient restClient;

    
    @Produces(MediaType.APPLICATION_JSON)     
    @POST
    @Path("oneclickenrollment")
    public TopSoapSelfQRes startSelfQ(SelfQRequest req) throws Exception {
        ...
        String soapResXML = restClient.CreateDossier(createDossier.getSoapBodyRequest()).await().indefinitely();
